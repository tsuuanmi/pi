...
| `tui` | `@earendil-works/pi-tui` | 0.79.6 | Terminal UI library with differential rendering, components, editor, keybindings, image/clipboard support. |
...
`scripts/` add ~5,187 lines of JS/MJS/TS tooling. AI scripts add 412 lines
(`generate-models.ts` 360, `generate-image-models.ts` 20,
`generate-test-image.ts` 32).
...
| `worker entry` | 1 | Spawned as a worker by the Bun build (`image-resize-worker.ts`) |
...
| `session-resources.ts` | 24 | Yes (leaf·2) | Tracks how many resources (tokens, images) a session has consumed, for accounting. |
...
| `image-models.ts` | 22 | Yes (leaf·1) | Access layer for image-generation models, the image counterpart of `models.ts`. |
| `image-models.generated.ts` | 6 | Yes (leaf·1) | Generated catalog of image models. |
| `images.ts` | 21 | Yes (leaf·1) | Small facade that exposes the image-generation API surface to consumers. |
| `images-api-registry.ts` | 53 | Yes (leaf·2) | Registry where image-generation providers register themselves. |
...
| `providers/images/register-builtins.ts` | 3 | Yes (leaf·1) | Registers the built-in image providers at startup. |
...
| Module | LOC | Used? | Key features |
| --- | ---: | --- | --- |
| `core/export-html/index.ts` | 281 | Yes (hub·96) | Exports a finished session as a self-contained HTML file. High fan-in entry point. |
| `core/export-html/ansi-to-html.ts` | 258 | Yes (leaf·2) | Converts terminal ANSI color codes into HTML spans. |
| `core/export-html/tool-renderer.ts` | 172 | Yes (leaf·2) | Renders tool calls and results as HTML for the export. |
...
| Module | LOC | Used? | Key features |
| --- | ---: | --- | --- |
| `utils/image/clipboard-image.ts` | 220 | Yes (4) | Reads image data from the clipboard (for pasting screenshots into the chat). |
| `utils/image/exif-orientation.ts` | 183 | Yes (leaf·2) | Reads EXIF orientation tags so pasted photos aren't sideways. |
| `utils/image/image-resize-core.ts` | 164 | Yes (leaf·2) | Core logic for resizing images before sending them to the model. |
| `utils/image/image-resize.ts` | 123 | Yes (5) | The resize entry point — picks the right path (worker vs sync) to resize an image. |
| `utils/image/image-resize-worker.ts` | 42 | worker entry | Worker that does the actual resizing off the main thread (entry for the Bun build). |
| `utils/image/photon.ts` | 139 | Yes (4) | Bindings to the Photon WASM library used for fast image resizing. |
| `utils/image/mime.ts` | 74 | Yes (leaf·2) | Detects an image's MIME type from its bytes. |
| `utils/image/image-convert.ts` | 41 | Yes (leaf·3) | Converts images between formats. |
...
| `terminal-image.ts` | 479 | Yes (10) | Renders images in the terminal (iTerm/Kitty protocols, sixel, or fallback). |
...
`stdin-buffer.test.ts` (458), `terminal-image.test.ts` (429), plus overlay/
...
`copy-assets` (themes, PNG assets, export-html templates/vendor, skills). The
Bun binary build (`build:binary`) compiles `dist/bun/cli.js` +
`image-resize-worker.ts` into a single `dist/pi` binary and copies binary
assets (incl. `photon_rs_bg.wasm`).
