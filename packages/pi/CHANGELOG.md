...
- **tui**: Removed `ThemeSelectorComponent` and `UserMessageComponent` from `@tsuuanmi/pi`; import them from `@tsuuanmi/pi-tui`.
...
- **tools**: Structured tool receipts now color status by severity and avoid repeating read paths and location summaries in the compact interactive view.
...
- Flattened tiny pi source folders into neighboring domains: binary entrypoint helpers under `cli`, event bus under `extensions`, API usage logging under `telemetry`, message optimization under `sdk`, provider helpers under `model`, and resource metadata under `package-manager`.
- Merged agent-session source modules into the existing `session` domain.

### Added

- Workflow agent tools (`subagent_spawn` / `subagent_status` / `subagent_await` / `subagent_steer` / `subagent_pause` / `subagent_resume` / `subagent_cancel`, `ralplan_run_agent`, `team_execute`, `team_resume`, `ultragoal_spawn_goal_agent`) are model-visible tools registered by the `@tsuuanmi/pi-workflows` extension. Generic and Ultragoal tools use this session's `SubagentManager`; team roles execute through the workflow-owned Orchestrator adapter. pi-agent owns runtime agents; pi-workflows owns turn order, guarded role checks, and result→artifact handoff.
...
- Workflow handoffs (`executeDeepInterviewWriteSpec`, `approveRalplanPlan`) now go through a single generic internal `handoffWorkflow` with callee->caller->active-state write order, both-side mode-state receipts (`handoff-send`/`handoff-receive`), and a transaction journal. The caller mode-state is now demoted to `active:false, current_phase:"handoff"` (previously deep-interview stayed `active:true` after handoff). `executeDeepInterviewWriteSpec` follows the two-step model: `finalizeDeepInterviewSpecState` persists the caller state with spec fields (`active:true`, `current_phase:"handoff"`, a regular write), then `handoffWorkflow` demotes it and promotes the callee; the `stop`/no-handoff branch is unchanged. Audit verbs map operations to the canonical set (`handoff-send`/`handoff-receive`->`handoff`; `force-repair`->`reconcile`; others->`write`/`clear`).
...
- Fixed bundled extension startup when `tsx` resolves imports from `@tsuuanmi/pi-agent/node` by adding explicit source and build path mappings.
