...
import { awaitRun, cancel, pause, resume, spawn, status, steer } from "#pi/subagents/tool-execution";
import type {
	SubagentAwaitInput,
	SubagentIdInput,
	SubagentResumeInput,
...
function contextOf(context: ExtensionContext): SubagentContext {
	if (!context.subagents) throw new Error("No subagent manager is available in this session.");
	return { manager: context.subagents, sessionId: context.sessionManager.getSessionId() };
}
