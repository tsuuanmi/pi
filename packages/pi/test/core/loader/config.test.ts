...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@tsuuanmi");
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@legacy-scope", "pi");
	mkdirSync(packageDir, { recursive: true });
	...
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(root, ".pnpm", "@legacy-scope+pi@0.0.0", "node_modules", "@legacy-scope", "pi", "dist", "cli.js"));
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@legacy-scope", "pi");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@legacy-scope", "pi", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@tsuuanmi/pi")).toBeUndefined();
		expect(getUpdateInstruction("@tsuuanmi/pi")).toBe(
			"Update @tsuuanmi/pi using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@tsuuanmi/pi");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@tsuuanmi/pi"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @tsuuanmi/pi`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@legacy-scope/pi", undefined, "@new-scope/pi");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
			display: `npm --prefix ${prefix} uninstall -g @legacy-scope/pi && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/pi`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@legacy-scope/pi"],
					display: `npm --prefix ${prefix} uninstall -g @legacy-scope/pi`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@tsuuanmi/pi", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @tsuuanmi/pi`,
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@tsuuanmi/pi"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @tsuuanmi/pi",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"pnpm remove -g @legacy-scope/pi && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/pi",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@legacy-scope/pi"],
					display: "pnpm remove -g @legacy-scope/pi",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/pi"],
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@tsuuanmi/pi";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@tsuuanmi", "pi");
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display: "yarn global remove @legacy-scope/pi && yarn global add --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@legacy-scope/pi"],
					display: "yarn global remove @legacy-scope/pi",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"bun uninstall -g @legacy-scope/pi && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/pi",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@legacy-scope/pi"],
					display: "bun uninstall -g @legacy-scope/pi",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/pi"],
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@tsuuanmi/pi")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@tsuuanmi/pi")).toContain("the install path is not writable");
	});
});
