...
import { executeBashWithOperations } from "#pi/exec/bash-executor";
import {
	createEditTool,
	createFindTool,
	createGrepTool,
...
describe("Pi Agent Tools", () => {
	...
	beforeEach(() => {
		...
		testDir = join(tmpdir(), `pi-test-${Date.now()}`);
...
describe("edit tool fuzzy matching", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `pi-fuzzy-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
...
describe("edit tool CRLF handling", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `pi-crlf-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
