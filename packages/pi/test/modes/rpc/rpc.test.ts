...
import type { AgentEvent } from "@tsuuanmi/pi-agent";
import { afterEach, beforeEach, describe, expect, test } from "vitest";
import { RpcClient } from "#pi/modes/rpc/client";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	beforeEach(() => {
		...
		client = new RpcClient({
			cliPath: join(__dirname, "../..", "dist", "cli", "cli.js"),
			cwd: join(__dirname, "../.."),
			env: { PI_AGENT_DIR: sessionDir },
			provider: "anthropic",
