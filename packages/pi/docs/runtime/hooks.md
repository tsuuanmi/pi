...
`registerWorkflows()` is the bundled composition helper. Workflow hooks own workflow behavior such as the Deep Interview mutation guard and HUD refresh. The Pi host supplies the session and UI context.

Workflow packages do not import Pi's application implementation. Pi satisfies the workflow host interfaces when it composes the built-in workflow extension.
