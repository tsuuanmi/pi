...
| --- | --- | --- |
| Resource/discovery ownership and provider precedence | `packages/pi/src/loader/resources.ts` | `src/agent/profiles.ts`, `src/skills/skills.ts`, `src/skills/prompt-templates.ts`, `src/package-manager/package-manager.ts` |
...
| Scoped live registry | `packages/pi/src/runtime/agent-session-services.ts` and `packages/pi/src/subagents/manager.ts` | `src/runtime/agent-session.ts`, `src/api/types.ts` |
