...
| Scoped live registry | `packages/pi/src/runtime/agent-session-services.ts` and `packages/pi/src/subagents/manager.ts` | `src/runtime/agent-session.ts`, `src/package-manager/extensions/types.ts` |
