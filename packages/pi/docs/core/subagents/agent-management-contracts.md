...
| --- | --- | --- |
| Resource/discovery ownership and provider precedence | `packages/pi/src/skills/resource-loader.ts` or a new `src/resource-providers.ts` | `src/agents/agent-profiles.ts`, `src/skills/skills.ts`, `src/skills/prompt-templates.ts`, `src/package-manager/package-manager.ts` |
...
| Scoped live registry | a new `packages/pi/src/agent-registry.ts` | `src/runtime/pi-session.ts`, `src/subagents/subagents.ts`, `src/api/types.ts` |
