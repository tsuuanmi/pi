...
Defined in [`session-manager.ts`](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/session/session-manager.ts):
...
Extensions can intercept and customize both compaction and branch summarization. See [`extensions/types.ts`](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/api/types.ts) for event type definitions.
