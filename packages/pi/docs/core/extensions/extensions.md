...
| Package | Purpose |
|---------|---------|
| `@tsuuanmi/pi` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
See [Session Format](../session-manager/session-format.md) for session storage internals and the SessionManager API.
...
Fired on compaction. See [compaction.md](../compaction/compaction.md) for details.
...
Fired on `/tree` navigation. See [Sessions](../session-manager/sessions.md) for tree navigation concepts.
...
Fired before each LLM call. Modify messages non-destructively. See [Session Format](../session-manager/session-format.md) for message types.
...
Transforms chain across handlers. See `streamingBehavior`-aware routing in `input-transform.ts` and `input-transform-streaming.ts`.
...
`true` in TUI and RPC modes. `false` in print mode (`-p`) and JSON mode. Use this to guard dialog methods (`select`, `confirm`, `input`, `editor`) and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) that work in both TUI and RPC modes. In RPC mode, some TUI-specific methods are no-ops or return defaults (see [RPC Mode](../../api/rpc.md#extension-ui-protocol)).
...
Read-only access to session state. See [Session Format](../session-manager/session-format.md) for the full SessionManager API and entry types.
...
```typescript
import { SessionManager } from "@tsuuanmi/pi";
...
```typescript
import type { AutocompleteItem } from "@tsuuanmi/pi-tui";
...
Register a keyboard shortcut. See [keybindings.md](../../modes/interactive/keybindings.md) for the shortcut format and built-in keybindings.
...
```typescript
import { withFileMutationQueue } from "@tsuuanmi/pi";
...
**Early termination:** Return `terminate: true` from `execute()` to hint that the automatic follow-up LLM call should be skipped after the current tool batch. This only takes effect when every finalized tool result in that batch is terminating. See `structured-output.ts` for a minimal example where the agent ends on a final structured-output tool call.
...
**Important:** Use `StringEnum` from `@tsuuanmi/pi-ai` for string enums. `Type.Union`/`Type.Literal` generates anyOf/const patterns that some providers don't support.
...
cp tool-override.ts ~/.pi/agent/extensions/
pi
```

See `tool-override.ts` for a complete example that overrides `read` with logging and access control.
...
- [read.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/edit.ts)
- [write.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/write.ts)
- [grep.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@tsuuanmi/pi";
...
```typescript
import { createBashTool } from "@tsuuanmi/pi";
...
See `ssh.ts` for a complete SSH example with `--ssh` flag.
...
See `truncated-tool.ts` for a complete example wrapping `rg` (ripgrep) with proper truncation.
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](../../ui/tui.md) for the full component API and [tool-execution.ts](https://github.com/tsuuanmi/pi/blob/main/packages/pi/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
**For custom components, see [tui.md](../../ui/tui.md)** which has copy-paste patterns for:
...
See `timed-confirm.ts` for complete examples.
...
```typescript
import { Text, Component } from "@tsuuanmi/pi-tui";
...
See [tui.md](../../ui/tui.md) for the full component API.
...
See [tui.md](../../ui/tui.md) for the full `OverlayOptions` and `OverlayHandle` API.
...
See [tui.md](../../ui/tui.md) Pattern 7 for a complete example with mode indicator.
...
```typescript
import { highlightCode, getLanguageFromPath } from "@tsuuanmi/pi";
...
| RPC (`--mode rpc`) | `"rpc"` | `true` | Dialogs and notifications via JSON protocol; `custom()` returns `undefined`. See [rpc.md](../../api/rpc.md) |
