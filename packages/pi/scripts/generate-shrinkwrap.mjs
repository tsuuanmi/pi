...
const piPackageDir = resolve(scriptDir, "..");
const rootLockfilePath = join(repoRoot, "package-lock.json");
const shrinkwrapPath = join(piPackageDir, "npm-shrinkwrap.json");
...
function generateShrinkwrap() {
	...
	const piPackage = readJson(join(piPackageDir, "package.json"));
	const internalWorkspaces = getInternalWorkspaces(lockPackages);
	const shrinkwrapPackages = {
		"": copyPackageJsonEntry(piPackage, { includeName: true }),
	};
	const addedPaths = new Set([""]);
	const internalNames = new Set();
	const queue = Object.keys(packageDependencies(piPackage)).map((name) => ({ name, from: "" }));
...
try {
	...
	if (checkOnly) {
		if (!existsSync(shrinkwrapPath)) {
			console.error("packages/pi/npm-shrinkwrap.json is missing.");
			console.error("Run: npm run shrinkwrap:pi");
			process.exit(1);
		}
		const current = readFileSync(shrinkwrapPath, "utf8");
		if (current !== content) {
			console.error("packages/pi/npm-shrinkwrap.json is out of date.");
			console.error("Run: npm run shrinkwrap:pi");
			process.exit(1);
		}
		console.log("packages/pi/npm-shrinkwrap.json is up to date.");
	} else {
		writeFileSync(shrinkwrapPath, content);
		const packageCount = Object.keys(shrinkwrap.packages).length - 1;
		const platformPackageCount = Object.values(shrinkwrap.packages).filter((entry) => entry.os || entry.cpu || entry.libc).length;
		console.log(
			`Wrote packages/pi/npm-shrinkwrap.json (${packageCount} packages, ${platformPackageCount} platform-specific).`,
		);
	}
} catch (error) {
