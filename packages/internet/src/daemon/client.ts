...
import type { BrowserInternetAccount, InternetControlAction } from "#internet/core/types";
import { controlHeaders, type DaemonConfig, daemonBaseUrl, readDaemonConfig } from "#internet/daemon/auth";
import { type CompactRequest, type CompactResponse, DAEMON_ROUTES, type DaemonHealth } from "#internet/daemon/routes";
...
export class DaemonClient {
	...
	private async request<T>(path: string, init: RequestInit, timeoutMs = this.timeoutMs): Promise<T> {
		...
		try {
			...
			throw new InternetError(`Internet browser daemon is unavailable at ${this.baseUrl()}`, {
