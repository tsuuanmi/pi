...
function inputContentParts(blocks: unknown[] | string | undefined): string | ContentPart[] {
...
function outputTextOf(blocks: unknown[] | string | undefined): TextContent[] {
...
function mapToolChoice(value: unknown): RequestOptions["toolChoice"] {
...
function buildTools(tools: unknown[] | undefined): Tool[] | undefined {
...
function ensureAssistantPlaceholder(messages: Message[], modelId: string, now: number): AssistantMessage {
  ...
  const placeholder: AssistantMessage = { role: "assistant", content: [], model: modelId, timestamp: now };
 ...
 */
function outputToToolResultContent(output: string | unknown[] | undefined): string | ContentPart[] {
...
function findToolById(messages: Message[], callId: string): { name: string; namespace?: string } {
...
export function parseRequest(body: unknown): ChatGptWebParsedRequest {
  ...
  const pendingReasoning: Array<{ part: ThinkingContent; envelopeSigned: boolean }> = [];
  // Assistant placeholder that folds pending reasoning into the same turn before tool calls.
  const assistantHolderWithReasoning = (): AssistantMessage => {
  ...
  // synthetic `{type:"compaction"}` output item (protocol/responses/compaction.ts). Flagged for the server.
