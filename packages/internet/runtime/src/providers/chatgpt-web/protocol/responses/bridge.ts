...
function responsesUsage(usage: Usage | undefined): Record<string, unknown> {
...
function responseError(status: number, type: string, message: string): RuntimeErrorPayload {
  return classifyError(status, type, message);
}

function adapterFailureFromEvent(event: Extract<AdapterEvent, { type: "error" }>): { httpStatus: number; error: RuntimeErrorPayload } {
  if (event.status === undefined && event.errorType === undefined && event.code === undefined) {
    return adapterFailureFromMessage(event.message);
  }
  const defaults = adapterFailureFromMessage(event.message);
  const httpStatus = event.status ?? defaults.httpStatus;
  const error = classifyError(httpStatus, event.errorType ?? defaults.error.type, event.message);
...
export function bridgeToResponsesSSE(
  ...
  options?: {
    ...
    onCompletedResponse?: (response: Record<string, unknown>, providerState?: ProviderContinuationState) => void;
  ...
  const emit = (name: string, data: Record<string, unknown>) => {
      ...
      let currentMsg: { itemId: string; outputIndex: number; text: string; phase?: MessagePhase } | null = null;
...
export function buildResponseJSON(
  ...
  options?: {
    ...
    compaction?: boolean;
    onProviderState?: (state: ProviderContinuationState) => void;
  },
  ...
  let currentText = "";
  let currentTextPhase: MessagePhase | undefined;
