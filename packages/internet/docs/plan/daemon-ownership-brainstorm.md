...
- `loginToChatGpt` (`src/browser/chatgpt-web/login.ts`) — the `login` subcommand — was not
  macOS-gated.
...
(`src/browser/chatgpt-web/login.ts:332-342`) and stores the authenticated session in a private
