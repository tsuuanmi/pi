...
> Status: **owned-daemon MVP plus R1–R3 implemented.** Fixed-effort models, explicit automatic-login
> settings, and read-only web search/fetch are current production scope.
...
settings, compaction, health/HUD, admin control, account routing, and read-only web search/fetch.
Claude/Gemini, hybrid capture, native Codex tool bridging, and non-Linux artifacts are deferred
