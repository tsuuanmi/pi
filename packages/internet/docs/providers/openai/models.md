...
All routes use concise provider-local ids (`light`, `medium`, `high`, `extra-high`, or `pro`), zero
cost, and `input: ["text", "image"]` with `reasoning: true`. Request adaptation maps
...
- Returns every Sol route filtered by `requiresPro` against `capabilities.proAvailable`.

### Context windows

`contextWindow` for a route is:

- `route.contextWindow` whenever `proAvailable` is false.
