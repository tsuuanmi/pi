...
function plist(config: RuntimeServiceConfig): string {
...
${args.map(arg => `    <string>${xml(arg)}</string>`).join("\n")}
  ...
  <dict>
    <key>PI_INTERNET_RUNTIME_HOME</key>
...
export function installService(config: RuntimeServiceConfig): ServiceStatus {
  assertMacOs();
...
export async function negotiateDrain(
  ...
  try {
    ...
    const activeAdapter = health.active_adapter_turns;
    if (!Number.isInteger(activeHttp) || !Number.isInteger(activeAdapter) || health.accepting_turns !== false) {
      throw new Error("daemon did not acknowledge the drain contract");
    }
    if ((activeHttp as number) > 0 || (activeAdapter as number) > 0) {
      throw new Error(`daemon has ${activeHttp} active HTTP turn(s) and ${activeAdapter} active adapter turn(s)`);
    }
...
async function acquireDrain(config: RuntimeServiceConfig): Promise<DrainLease> {
  if (!getServiceStatus().loaded) return { release: async () => {} };
  return negotiateDrain(action => requestControl(config, action));
}
...
export async function assertServiceIdle(config: RuntimeServiceConfig): Promise<void> {
  const lease = await acquireDrain(config);
  await lease.release();
}

export async function restartService(config: RuntimeServiceConfig): Promise<ServiceStatus> {
  assertMacOs();
...
export async function stopService(config: RuntimeServiceConfig): Promise<ServiceStatus> {
  assertMacOs();
...
export async function uninstallService(config: RuntimeServiceConfig): Promise<ServiceStatus> {
  assertMacOs();
  if (getServiceStatus().loaded) {
    const lease = await acquireDrain(config);
    try {
