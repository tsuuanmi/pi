...
import { extractChatGptTurnIdentity, extractChatGptTurnUserRevision } from "../turn/environment";
...
const checkpointSchema = textCheckpointSchema;
...
function currentTurnBoundary(parsed: ParsedRequest, input: unknown[], turnId: string): number | undefined {
...
function parentAssistantAnswer(parsed: ParsedRequest, turnId: string): string | undefined {
...
function currentTurnInput(parsed: ParsedRequest, turnId: string): unknown[] | undefined {
  const body = record(parsed._rawBody);
  const input = Array.isArray(body?.input) ? body.input : undefined;
  if (!input) return undefined;
  const boundary = currentTurnBoundary(parsed, input, turnId);
...
export class ChatGptLunaCheckpointStore {
  ...
  apply(parsed: ParsedRequest): { parsed: ParsedRequest; applied: boolean; reason?: string } {
  ...
  commit(parsed: ParsedRequest, captured: CapturedChatGptLunaCheckpoint, answer: string): void {
    const identity = extractChatGptTurnIdentity(parsed);
