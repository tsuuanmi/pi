...
async function proxyCheck(config: AppConfig): Promise<DoctorCheck> {
  ...
  try {
    ...
    if (body.service !== "pi-internet-runtime" || body.status !== "ok") {
...
export async function runDoctor(): Promise<DoctorReport> {
  ...
  if (!browserLoginStateExists(config)) {
    checks.push({ id: "login", status: "error", message: "ChatGPT login state is missing or unverified; run `pi-internet-runtime login`" });
