...
import { namespacedToolName, type AdapterEvent, type ContentPart, type ParsedRequest, type ProviderConfig, type ToolResultMessage, type Usage } from "./types";
...
function brokerContent(content: string | ContentPart[]): unknown[] {
...
function brokerResult(message: ToolResultMessage): BrokerToolResult {
...
function emitToolBatch(requests: BrokerToolRequest[], usage: Usage, emit: (event: AdapterEvent) => void): void {
...
function emitBrowserCompletion(outcome: ChatGptBrowserOutcome, usage: Usage, emit: (event: AdapterEvent) => void): void {
...
export function createChatGptWebAdapter(provider: ProviderConfig): ProviderAdapter {
  ...
  const currentUsageInput = (parsed: ParsedRequest): ParsedRequest => (
