...
export function setupProxyIsReady(
  ...
  return health.service === "pi-internet-runtime"
    && health.status === "ok"
...
async function bootstrapTunnelProfile(config: AppConfig): Promise<void> {
  ...
  try {
    ...
    // stopping the validation runtime. The background service reconnects the committed profile.
...
export async function setup(options: SetupOptions): Promise<SetupResult> {
  ...
  if (process.platform !== "darwin") {
    throw new Error("Managed Chrome setup currently requires macOS.");
  }
  ...
  if (beforeService.loaded && !existing) {
    throw new Error("A pi-internet-runtime service is loaded but its configuration is missing; refusing to replace an unverifiable process");
  }

  let loginCreated = false;
  const stored = storedBrowserLoginCapabilities(config);
  let solAvailable = stored.solAvailable;
  let proAvailable = stored.proAvailable;
  ...
  if (beforeService.loaded && preliminaryChange && !options.restartService) {
    throw new Error(
      "The daemon is currently serving a browser turn and setup would change its runtime. "
      + "Rerun from a normal terminal with --restart-service after the active task finishes.",
    );
  }
  ...
  if (config.mode === "full") {
    ...
    const needsServiceInstall = !tunnelService.installed || !tunnelService.loaded || !tunnelServiceDefinitionMatches(config);
    if (needsServiceInstall || needsProfile) {
