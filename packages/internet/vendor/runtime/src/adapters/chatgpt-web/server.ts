...
import { bridgeToResponsesSSE, buildResponseJSON, formatErrorResponse } from "./responses/bridge";
import type { AppConfig } from "./config";
import { providerConfig } from "./config";
import { AsyncEventQueue } from "../../event-queue";
import { readJsonRequestBody } from "../../http-body";
import { httpStatusFromTerminalError } from "./responses/errors";
...
import { namespacedToolName, type AdapterEvent, type ParsedRequest } from "./types";
...
export function startServer(
  ...
  const activity = () => ({
    ...
    active_adapter_turns: chatGptTurnSessions.activeCount(),
  });
  ...
  const server = startHttpServer({
    ...
    fetch(req) {
      ...
      if (req.method === "POST" && url.pathname === "/admin/shutdown") {
        ...
        if (!draining || current.active_http_turns > 0 || current.active_adapter_turns > 0) {
          return Response.json(
            {
              status: "refused",
              accepting_turns: !draining,
      ...
      if (req.method === "GET" && url.pathname === "/v1/models") {
        if (draining) {
          return formatErrorResponse(
            503,
            "server_error",
            "pi-internet-runtime is draining for a requested service operation",
          );
        }
        return httpTurns.track(async () => {
      ...
      if (req.method === "POST" && url.pathname === "/v1/responses") {
        if (draining) return formatErrorResponse(503, "server_error", "pi-internet-runtime is draining for a requested service operation");
  ...
  function scheduleIdleShutdown(): void {
    ...
    idleTimer = setTimeout(() => {
      const current = activity();
      if (current.active_http_turns > 0 || current.active_adapter_turns > 0) {
    ...
    shutdownPromise = (async () => {
      ...
      if (failures.length > 0) {
        ...
        for (const failure of failures) {
          console.error(`[pi-internet-runtime] shutdown cleanup failed: ${failure instanceof Error ? failure.message : String(failure)}`);
        }
      }
      ...
      process.exitCode = 1;
      console.error(`[pi-internet-runtime] server shutdown failed: ${error instanceof Error ? error.message : String(error)}`);
    });
  }
