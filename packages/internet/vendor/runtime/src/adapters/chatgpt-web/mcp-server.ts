...
import { namespacedToolName, type Tool } from "./types";
...
function wireName(tool: Tool): string {
  return namespacedToolName(tool.namespace, tool.name);
}

function exactTool(environment: ChatGptTurnEnvironment, name: string): Tool | undefined {
  return environment.tools.find(tool => !tool.namespace && tool.name === name);
}

function namedTool(environment: ChatGptTurnEnvironment, requestedWireName: string): Tool {
...
function execGateway(environment: ChatGptTurnEnvironment): Tool | undefined {
