...
export async function detectChatGptAccountCapabilities(
  ...
  while (true) {
    ...
    if (composerReady && formReady && documentReady) {
      ...
      if (Date.now() - absenceSince >= stableAbsenceMs) {
        throw new Error("ChatGPT account does not expose the Sol model selector");
      }
    } else {
      absenceSince = undefined;
    }
  ...
  try {
    ...
    try {
      ...
      if (ready === "items") {
        return { proAvailable: await efforts.count() >= 5 };
      }
      ...
      return { proAvailable: state.max - state.min + 1 >= 5 };
    } finally {
      waitAbort.abort();
    }
  } finally {
