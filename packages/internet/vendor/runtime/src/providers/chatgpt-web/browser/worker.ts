 ...
 * on the current ChatGPT composer. This chunks only the input event; the resulting user message
...
export class ChatGptBrowserWorker {
  ...
  private readonly turns = new BrowserTurnRunner<string>({ maxConcurrent: MAX_CHATGPT_BROWSER_TABS, label: "ChatGPT Web" });
  ...
  run(turn: BrowserTurn): Promise<string> {
    return this.turns.run(turn.traceId, () => this.runBrowserTurn(turn));
  }
  ...
  private enqueueMaintenance<T>(name: string, action: () => Promise<T>): Promise<T> {
    return this.turns.enqueueMaintenance(name, action);
  }
  ...
  private pageForConversation(turn: BrowserConversationTurn): Promise<Page> {
    return this.session.pageForKey(turn.threadId, MAX_CHATGPT_BROWSER_TABS);
  }
