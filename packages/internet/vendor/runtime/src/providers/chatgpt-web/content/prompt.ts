 ...
 * product request still has room for its own message metadata. This applies only to compaction:
 * native Codex also removes the
...
export function countChatGptContextImages(messages: readonly Message[]): number {
  let total = 0;
...
function assistantContent(content: AssistantContentPart[]): unknown[] {
...
function plainMessageText(message: Message): string | undefined {
...
function startsWithControlBlock(message: Message, tag: string): boolean {
 ...
 * touched.
 */
export function withoutSupersededModelSwitchContracts(messages: readonly Message[]): Message[] {
...
export function compileChatGptWebPrompt(
  ...
  const build = (sourceMessages: readonly Message[]): CompiledChatGptWebPrompt => {
