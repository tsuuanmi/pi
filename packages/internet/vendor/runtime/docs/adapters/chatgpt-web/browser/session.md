# adapters/chatgpt-web/browser/session

Mirrors `src/adapters/chatgpt-web/browser/session.ts`.
...
The implementation in `src/adapters/chatgpt-web/browser/session.ts` is authoritative. Update this page when its exported API, data flow, or safety behavior changes.
