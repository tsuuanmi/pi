...
Mirrors `src/adapters/chatgpt-web/browser/worker.ts`.
...
- `adapters/chatgpt-web/conversation/canary.ts`
- `adapters/chatgpt-web/content/markdown.ts`
...
- `adapters/chatgpt-web/transport/wire-capture.ts`

## Source of truth

The implementation in `src/adapters/chatgpt-web/browser/worker.ts` is authoritative. Update this page when its exported API, data flow, or safety behavior changes.
