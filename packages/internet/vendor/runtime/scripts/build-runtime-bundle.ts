...
import { VERSION } from "../src/core/config";
...
function embeddedBunExecutable(): string {
  const configured = process.env.PI_INTERNET_RUNTIME_EMBEDDED_BUN;
  if (!configured) return realpathSync(process.execPath);
  if (!isAbsolute(configured)) throw new Error("PI_INTERNET_RUNTIME_EMBEDDED_BUN must be an absolute path");
...
const launcherName = process.platform === "win32" ? "pi-internet-runtime.cmd" : "pi-internet-runtime";
const launcher = process.platform === "win32" ? `@echo off
setlocal
chcp 65001 >nul
set "ROOT=%~dp0.."
set "PI_INTERNET_RUNTIME_LAUNCHER=%~f0"
...
export PI_INTERNET_RUNTIME_LAUNCHER="$invoked"
