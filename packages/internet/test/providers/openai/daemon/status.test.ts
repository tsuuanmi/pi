...
import { readDaemonStatusSnapshot } from "#internet/providers/openai/daemon/status";

describe("readDaemonStatusSnapshot", () => {
	it("returns an unavailable snapshot instead of throwing", async () => {
		vi.spyOn(AccountRegistry.prototype, "getOpenAi").mockRejectedValue(new Error("missing"));
