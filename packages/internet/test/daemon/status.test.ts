...
import { readDaemonStatusSnapshot } from "#internet/daemon/status";

describe("readDaemonStatusSnapshot", () => {
	it("returns an unavailable snapshot instead of throwing", async () => {
		vi.spyOn(AccountRegistry.prototype, "getBrowser").mockRejectedValue(new Error("missing"));
	...
	it("uses the registry account rather than external daemon environment", async () => {
		vi.spyOn(AccountRegistry.prototype, "getBrowser").mockResolvedValue({ id: "default" } as never);
