...
- `RunTeamResult`: lifecycle status, success flag, optional abort reason, final task snapshots, task metrics, and concatenated completed-task output.
...
- `OrchestratorCheckpointSnapshot`: persisted checkpoint payload containing queue state, metrics, task-start count, and abort metadata.
