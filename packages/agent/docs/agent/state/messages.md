...
`src/state/messages.ts` defines agent-specific message roles and conversion helpers used before sending context to the LLM.
...
The module augments `CustomAgentMessages` in `src/state/state.ts` so these roles are part of the package `AgentMessage` union.
