...
import { calculateCost } from "#ai/model/catalog";
import type { AnthropicMessagesCompat, Model, StreamFunction } from "#ai/model/index";
...
import { buildBaseOptions } from "#ai/provider/openai/stream-options";
...
export interface AnthropicOptions extends StreamOptions {
	 ...
	 * (adaptive thinking).
	 * Default: undefined (thinking is omitted unless `streamAnthropic()` maps
	 * a stream reasoning level to this option, or callers set it explicitly).
	 */
	 ...
	 * Default: omitted unless `streamAnthropic()` maps a stream reasoning
	 ...
	 * custom clients that share the same messaging API.
	 */
	client?: Anthropic;
}
...
const streamAnthropicWithProviderOptions: StreamFunction<"anthropic-messages", AnthropicOptions> = (
	model: Model<"anthropic-messages">,
	context: Context,
		...
		try {
			...
			for await (const event of iterateAnthropicEvents(response, options?.signal)) {
				if (event.type === "message_start") {
					...
					output.usageProvenance = { type: "provider_reported", fields: Object.keys(event.message.usage) };
					...
					output.usageProvenance = { type: "provider_reported", fields: Object.keys(event.usage) };
					// Anthropic doesn't provide total_tokens, compute from components
					output.usage.totalTokens =
						output.usage.input + output.usage.output + output.usage.cacheRead + output.usage.cacheWrite;
					calculateCost(model, output.usage);
...
export const streamAnthropic: StreamFunction<"anthropic-messages", StreamOptions> = (
	model: Model<"anthropic-messages">,
	context: Context,
	...
	if (!options?.reasoning) {
		return streamAnthropicWithProviderOptions(model, context, {
			...base,
