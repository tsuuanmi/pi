...
import { calculateCost, clampThinkingLevel } from "#ai/model/catalog";
import type { Model, OpenAICompletionsCompat, StreamFunction } from "#ai/model/index";
import { parseStreamingJson, sanitizeSurrogates } from "#ai/parsing/json-parser";
import type { TextContent, ThinkingContent, ToolCall } from "#ai/protocol/content";
import type { Context } from "#ai/protocol/context";
import type { AssistantMessage, Message, StopReason, ToolResultMessage } from "#ai/protocol/message";
import type { CacheRetention, ProviderEnv, StreamOptions } from "#ai/protocol/options";
import type { Tool } from "#ai/protocol/tool";
import { getProviderEnvValue } from "#ai/provider/config";
import { buildBaseOptions, clampOpenAIPromptCacheKey } from "#ai/provider/openai/stream-options";
import { transformMessages } from "#ai/provider/openai/transform-messages";
...
const streamOpenAICompletionsWithProviderOptions: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		try {
			...
			if (output.stopReason === "error") {
				throw new Error(output.errorMessage || "ProviderId returned an error stop reason");
			}
			if (!hasFinishReason) {
...
export const streamOpenAICompletions: StreamFunction<"openai-completions", StreamOptions> = (
	...
	return streamOpenAICompletionsWithProviderOptions(model, context, {
		...base,
		reasoningEffort,
		toolChoice,
...
function buildParams(
	...
	if (compat.thinkingFormat === "string-thinking" && model.reasoning) {
...
export function convertMessages(
	...
	const normalizeToolCallId = (id: string): string => {
		...
		// These come from providers like openai-codex.
		...
		if (msg.role === "user") {
			...
			for (; j < transformedMessages.length && transformedMessages[j].role === "toolResult"; j++) {
				...
				const toolResultMsg: ChatCompletionToolMessageParam = {
					role: "tool",
					content: sanitizeSurrogates(textResult || "(no output)"),
...
function parseChunkUsage(
	...
	// tokens (hits). OpenAI does not document or emit cache_write_tokens.
...
function mapStopReason(reason: ChatCompletionChunk.Choice["finish_reason"] | string): {
	...
	switch (reason) {
		...
		case "content_filter":
			return { stopReason: "error", errorMessage: "ProviderId finish_reason: content_filter" };
		case "network_error":
			return { stopReason: "error", errorMessage: "ProviderId finish_reason: network_error" };
		default:
			return {
				stopReason: "error",
				errorMessage: `ProviderId finish_reason: ${reason}`,
			};
	}
}
