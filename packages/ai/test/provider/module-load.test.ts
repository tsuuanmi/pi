...
describe("provider module loading", () => {
	...
	it("loads only the Anthropic SDK when streaming a custom Anthropic model", () => {
		const result = runProbe(`
			const model = {
			...
			await mod.stream(model, context).result();
		`);

		expect(result.loadedSpecifiers).toEqual(["@anthropic-ai/sdk"]);
	});

	it("loads only the Anthropic SDK when dispatching through stream", () => {
		const result = runProbe(`
