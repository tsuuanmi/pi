...

{
	...
	describe("openai-codex streaming", () => {
		...
		it("preserves gpt-5.5 xhigh reasoning effort from stream options", async () => {
			const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
			process.env.PI_AGENT_DIR = tempDir;
			const token = mockToken();
		...
		it("forwards auto transport from stream options and uses cached websocket context", async () => {
			const token = mockToken();
