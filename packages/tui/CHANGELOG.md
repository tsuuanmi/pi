...
- **status-line**: Renamed status-line segment groups to `modelSegments`/`environmentSegments` and separator glyphs to `before`/`after` for the semantic multi-row layout; the default environment row now contains only Git and path.
...
- **tui**: Structured tool receipts now color status by severity and omit redundant location summaries from the compact view.
...
- Removed bundled native modifier binaries and the `isNativeModifierPressed()` helper.
...
- Exported the reusable status line, HUD components, and workflow HUD registration/building helpers from TUI.
