...
import type { AgentSessionContext } from "#coding-agent/agent-session/agent-session-context";
import {
...
} from "#coding-agent/agent-session/model-control";
import { expandSkillCommand } from "#coding-agent/agent-session/skill-expansion";
import { computeContextUsage, computeSessionStats } from "#coding-agent/agent-session/stats-export";
import {
...
import { type BashResult, executeBashWithOperations } from "#coding-agent/exec/bash-executor";
import {
	type ContextUsage,
...
import { installAgentToolHooks } from "#coding-agent/hooks/tool-hooks";
...
import { expandPromptTemplate, type PromptTemplate } from "#coding-agent/skills/prompt-templates";
import type { ResourceExtensionPaths, ResourceLoader } from "#coding-agent/skills/resource-loader";
...
export interface AgentSessionConfig {
	...
	/** Initial active built-in tool names. Default: [read, bash, edit, write, lsp] */
...
export class AgentSession {
				...
				if (isOAuth) {
					throw new Error(
						`Authentication failed for "${this.model.provider}". ` +
							`Credentials may have expired or network is unavailable. ` +
							`Run '/account add ${this.model.provider}' to re-authenticate.`,
					);
				}
	...
	private _expandSkillCommand(text: string): string {
		return expandSkillCommand(text, this._ctx());
	}
