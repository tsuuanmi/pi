...
```bash
npm install @tsuuanmi/pi-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@tsuuanmi/pi-coding-agent";
...
import { AuthStorage, ModelRegistry } from "@tsuuanmi/pi-coding-agent";
...
const { session } = await createAgentSession({
  ...
  // Optional scoped models for startup/model resolution
  scopedModels: [
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@tsuuanmi/pi-coding-agent";
...
import { createAgentSession, defineTool } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@tsuuanmi/pi-coding-agent";
...
See [RPC documentation](../../api/rpc.md) for the JSON protocol.
