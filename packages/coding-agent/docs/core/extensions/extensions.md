...
| Package | Purpose |
|---------|---------|
| `@tsuuanmi/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
```typescript
import { isToolCallEventType } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { createBashTool } from "@tsuuanmi/pi-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](../../ui/tui.md) for the full component API and [tool-execution.ts](https://github.com/tsuuanmi/pi/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@tsuuanmi/pi-coding-agent";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@tsuuanmi/pi-coding-agent";
