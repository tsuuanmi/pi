...
describe("detectInstallMethod", () => {
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@tsuuanmi/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@tsuuanmi/pi-coding-agent")).toBe(
			"Update @tsuuanmi/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@tsuuanmi/pi-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@tsuuanmi/pi-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @tsuuanmi/pi-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@tsuuanmi/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @tsuuanmi/pi-coding-agent`,
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@tsuuanmi/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @tsuuanmi/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@tsuuanmi/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@tsuuanmi", "pi-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@tsuuanmi/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@tsuuanmi/pi-coding-agent")).toContain(
