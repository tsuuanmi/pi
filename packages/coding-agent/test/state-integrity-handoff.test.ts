...
import { transactionJournalPath, workflowActiveStatePath } from "../src/workflows/shared/paths.ts";
import { clearWorkflowState, readWorkflowState, writeWorkflowState } from "../src/workflows/shared/workflow-state.ts";

async function readEnvelope(
	cwd: string,
	skill: "deep-interview" | "ralplan" | "ultragoal",
): Promise<Record<string, unknown>> {
	const state = await readWorkflowState(cwd, skill);
...
async function readActiveEntries(cwd: string): Promise<Record<string, unknown>[]> {
...
describe("state-integrity transaction-backed handoff (STATE-006)", () => {
	...
	it("deep-interview -> ralplan: both-side receipts share mutationId, caller demoted, callee promoted, journal removed", async () => {
		...
		const entries = await readActiveEntries(cwd);
	...
	it("ralplan -> ultragoal: callee promoted to its initial phase with source metadata", async () => {
		await writeWorkflowState(cwd, "ralplan", { active: true, current_phase: "pending-approval", run_id: "run-1" });
	...
	it("rejects a handoff to a callee that already holds an active handoff from this caller", async () => {
		...
		await writeWorkflowState(cwd, "ralplan", {
			active: true,
	...
	it("allows re-handoff after the callee has been cleared (active:false)", async () => {
		...
		await clearWorkflowState(cwd, "ralplan");
