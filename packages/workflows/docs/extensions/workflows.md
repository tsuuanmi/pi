...
- The default extension registers in-process workflow tools, event observers, and control hooks for an interactive Pi session. It does not register or invoke the CLI command.
...
- `registerWorkflowHooks(host)` from `@tsuuanmi/pi-workflows/hooks` registers HUD refresh event observers and workflow policy hooks, including the Deep Interview mutation guard for `edit`, `write`, and `bash`.
...
`@tsuuanmi/pi-workflows/tool` remains the lower-level tool registration helper for custom hosts that need tools without lifecycle integration.
...
Workflow lifecycle registration lives in `src/hooks.ts` and accepts `Pick<ExtensionAPI, "on" | "onHook">`. Observer registrations (`session_start`, `turn_end`, `tool_execution_end`) are separate from policy hooks (`before_agent_start`, `tool_call`, `tool_result`). Payload, result, and context types come directly from Pi; Workflows owns no duplicate extension lifecycle contract. The generic Agent hook mechanism remains in `@tsuuanmi/pi-agent`.

The package extension passes the host context to workflow tool and lifecycle registrars and does not implement workflow policy. Generic agent declarations come from `@tsuuanmi/pi-agent`; session-aware subagent declarations and execution come from `@tsuuanmi/pi-orchestrator`.
