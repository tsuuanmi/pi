...
| `src/skills/ralplan/` | Ralplan `SKILL.md` plus role definitions and request construction, Pi subagent and Orchestrator integration, checkpoint and agent-record persistence, planning runtime, policy, guards, gates, obstacles, verdicts, help/surface metadata, tools, HUD, orchestration snapshots, expected-action selection, and journaled completion transactions. | [skills/ralplan/index.md](skills/ralplan/index.md) |
...
| `src/tool/` | Workflow context, host/spec contracts, registration, and workflow surface metadata. | [subagents/subagents.md](subagents/subagents.md) |
