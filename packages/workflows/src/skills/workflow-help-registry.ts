...
export function renderWorkflowSkillHelp(skill: WorkflowSkill): string {
	...
	return `Usage:\n  pi workflow ${help.skill} <action> (--input '{...}' | --input-file ./payload.json) [--json]\n\n${help.label} agent flow:\n${help.agentFlow.map((step, index) => `  ${index + 1}. ${step}`).join("\n")}\n\n${help.label} actions:\n${skillActions}\n\nAction details and parameters:\n${actionDetails}\n\nInput rules:\n  - Commands accept a JSON object with --input or --input-file.\n  - Every skill action requires the canonical sessionId; tools obtain it from the host session context.\n  - Use pi workflow state ${help.skill} read|clear|doctor for inspection and explicit clearing; use skill actions for workflow mutations.\n\nDocs:\n${docs}\n`;
}
