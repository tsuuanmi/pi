...
- Planner, Architect, and Critic passes must use `pi workflow ralplan run-agent`; do not simulate all roles inline in the parent conversation.
...
4. Run the Planner with `pi workflow ralplan run-agent` using `role: "planner"`, `stage: "planner"`, and `stageN: 1`. The role agent must create and persist a planner artifact containing:
...
6. Run the Architect with `pi workflow ralplan run-agent` using `role: "architect"`, `stage: "architect"`, and the planner artifact path in `contextArtifacts`. It must review for:
...
7. Run the Critic with `pi workflow ralplan run-agent` using `role: "critic"`, `stage: "critic"`, and planner/architect artifact paths in `contextArtifacts`. It must evaluate:
...
8. If the critic requests iteration, run a Planner revision with `pi workflow ralplan run-agent` using `role: "planner"`, `stage: "revision"`, and consolidated Architect/Critic feedback. Then repeat Architect/Critic review. Cap at five iterations.
