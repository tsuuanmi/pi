...
export async function approveRalplanPlan(
	...
	if (!status.run_id)
		throw new Error(
			"cannot approve ralplan without a run_id: no artifacts have been persisted yet. Run the planner through `subagent_spawn` and `pi workflow ralplan write-artifact` first, then run `pi workflow ralplan doctor` if the run still looks inconsistent.",
		);
